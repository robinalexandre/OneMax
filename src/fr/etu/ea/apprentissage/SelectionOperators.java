...
public abstract class SelectionOperators {
	...
	public void addLastPerformance(float bestFitness, int indexOperateur) { //Normaliser ici
		for(int j = 0; j < this.gains.size(); j++) {
			if(j == indexOperateur) {
				float gain = bestFitness - this.bestFitness;
				if(gain > 0) {
					//System.out.println("Iteration: " + this.iteration + "\nGain:" + gain + "operateur: " + j);
			...
			this.utility[j] = this.computeUtility(j, totalGain/(this.iteration <= 100 ? this.iteration-1 : gainsOperators.length));
		}
	}
...
//	private void computeDistanceHamming(Population population) {
//		float distanceHammingTotal = 0;
//		float distanceHammingCouple;
//		for (int i = 0; i < population.getIndividus().length/2; i+=2) {
//			distanceHammingCouple = 0;
//			for(int j = 0; j < population.getIndividus()[i].getBits().length; j++) {
//				if(population.getIndividus()[i].getBits()[j] != population.getIndividus()[i+1].getBits()[j])
//					distanceHammingCouple++;
//			}
//			if(distanceHammingCouple != 0)
//				distanceHammingCouple = (distanceHammingCouple/population.getIndividus()[i].getBits().length);
//			distanceHammingTotal += distanceHammingCouple;
//		}
//		this.alpha = distanceHammingTotal/(population.getIndividus().length/2);
//	}
	
	protected void newIteration(Population population) {
		//this.computeDistanceHamming(population);
		this.iteration++;
	}
